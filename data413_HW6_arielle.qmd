...
```{r}
#NRC joy words, joins with emmas text
...
```{r}
#AFIN positive score above 1, joins with emmas text
...
```{r}
#BING positive, joins with emmas text 
...
```{r}
#addition of reorder so the bar chart is by frequency not alphabetical 

#NRC Bar chart
ggplot(nrc_freq, aes(x = reorder(word, n), y = n)) +
  geom_col(fill = "steelblue") +
  labs(title = "NRC Joy Words in Emma (n > 50)",x = "Word", y = "Frequency")

#AFINN Bar chart
ggplot(afinn_freq, aes(x= reorder(word,n), y=n))+
  geom_col(fill= "plum")+
  labs(title= "AFINN Positive Words in Emma (n >50)", x= "Word", y= "Frequency")

#BING Bar Chart
ggplot(bing_freq, aes(x= reorder(word,n), y=n))+
  geom_col(fill= "gold2")+
  labs(title= "BING Positive Words in Emma (n >50)", x= "Word", y= "Frequency")
...
wordcloud(words= emma_nrc_counts$word, freq = emma_nrc_counts$n, min.freq= 20,
          colors= sample(colors(),40), random.order = FALSE)

#AFINN word cloud 
wordcloud(words= emma_afinn_counts$word, freq = emma_afinn_counts$n, min.freq= 20,
          colors= sample(colors(),40), random.order = FALSE)

#BING word cloud
wordcloud(words= emma_bing_counts$word, freq = emma_bing_counts$n, min.freq= 20,
          colors= sample(colors(),40), random.order = FALSE)
```
